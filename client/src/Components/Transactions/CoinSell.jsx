...
export default function CoinSell() {
  ...
  // Calculate max value user can get from selling all
  const maxSellValue = userHoldings * getPriceInINR();
  ...
  const getusertransaction_byQuantity = async () => {
    ...
    if (Number(Quantity) > userHoldings) {
      alert(`Insufficient holdings!\nYou're trying to sell ${Quantity} but you only have ${userHoldings.toFixed(8)} ${data?.symbol?.toUpperCase()}`);
      return;
    }
    ...
    if (response.data === "NO") {
      alert("Transaction failed: Insufficient holdings");
  ...
  const getusertransaction_byAmount = async () => {
    ...
    const requiredQuantity = amountInINR / getPriceInINR();
    if (requiredQuantity > userHoldings) {
      alert(`Insufficient holdings!\nTo receive ${formatCurrency(amountInINR)}, you need ${requiredQuantity.toFixed(8)} ${data?.symbol?.toUpperCase()}\nYou only have ${userHoldings.toFixed(8)}`);
      return;
    }
  ...
  // Sell all button
  const handleSellAll = () => {
    setQuantity(userHoldings.toString());
  };
      ...
      {showSuccess && (
        <div className="fixed inset-0 bg-black bg-opacity-70 flex items-center justify-center z-50">
          <div className="bg-[#1d2230] p-8 rounded-xl text-center animate-pulse">
            ...
            <div className="text-white text-2xl font-bold mb-2">Sale Successful!</div>
            <div className="text-[#26a69a] text-lg">
              You sold {Quantity || Quantity_for_amount?.toFixed(8)} {data?.name}
            </div>
            <div className="text-[#f59e0b] text-lg mt-2">
              +{Amount || formatCurrency(Amount_for_amount)}
            </div>
        ...
        <div className="bg-[#1d2230] rounded-xl p-6">
          ...
          <div className="bg-[#272e41] rounded-lg p-4 mb-6 grid grid-cols-2 md:grid-cols-3 gap-4 text-center">
            ...
            <div className="col-span-2 md:col-span-1">
              <div className="text-[#9ca3af] text-sm">Max Sale Value</div>
              <div className="text-[#26a69a] font-bold text-lg">{formatCurrency(maxSellValue)}</div>
            </div>
          </div>

          {userHoldings <= 0 && (
            <div className="bg-[#c12f3d20] border border-[#c12f3d] rounded-lg p-4 mb-6 text-center">
              <div className="text-[#c12f3d] font-bold">⚠️ You don't own any {data?.name}</div>
              <div className="text-gray-400 mt-2">Buy some first before you can sell!</div>
            </div>
          )}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="bg-[#171b26] rounded-lg p-6 text-white text-center">
              ...
              <div className="text-[#c12f3d] text-2xl font-bold mb-4">{currprise}</div>
            ...
            <div className="grid grid-cols-1 gap-4">
              ...
              <div className="bg-[#171b26] rounded-lg p-4 text-white">
                <div className="text-center font-bold text-lg mb-4 text-[#c12f3d]">
                  Sell by Quantity
                </div>
                <div className="space-y-3">
                  <div>
                    <div className="flex justify-between items-center mb-1">
                      ...
                      <button
                        ...
                        className="text-xs bg-[#c12f3d] hover:bg-[#a02632] text-white px-2 py-1 rounded"
                      >
                        Sell All
                      </button>
                    </div>
                    <input
                      ...
                      className="w-full bg-[#272e41] text-white p-3 rounded-lg border border-[#3d4657] focus:border-[#c12f3d] outline-none"
                  ...
                  <button
                    ...
                    disabled={userHoldings <= 0}
                    className={`w-full font-bold py-3 rounded-lg transition-all ${userHoldings > 0
                      ? "bg-[#c12f3d] hover:bg-[#a02632] text-white"
              ...
              {/* Sell by Amount */}
              <div className="bg-[#171b26] rounded-lg p-4 text-white">
                <div className="text-center font-bold text-lg mb-4 text-[#f59e0b]">
                  Sell to Get Amount ({currencyINR ? "₹ INR" : "$ USD"})
                </div>
                <div className="space-y-3">
                  <div>
                    <label className="text-sm text-gray-400">Amount you want ({currencyINR ? "₹" : "$"})</label>
                    <input
                      type="number"
                      step="any"
                      min="0"
                      ...
                      className="w-full bg-[#272e41] text-white p-3 rounded-lg border border-[#3d4657] focus:border-[#f59e0b] outline-none"
                      placeholder={`Enter amount in ${currencyINR ? "INR" : "USD"}`}
                      disabled={userHoldings <= 0}
                    />
                  </div>
                  <div className="flex justify-between items-center bg-[#272e41] p-3 rounded-lg">
                    <span className="text-gray-400">You'll Sell:</span>
                    <span className="text-[#c12f3d] font-bold">
